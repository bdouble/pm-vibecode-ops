...
**Code-review-specific obligation:** The code-reviewer agent ALSO inspects prior phase reports for `SCOPE_GAP` — work that should have been done per the AC but is missing from the implementation. Treat any SCOPE_GAP finding as a Pass 1 failure and re-dispatch the relevant prior phase (NOT just request changes to the current diff).
...
2. **Context Loading**: Load implementation branch, architectural guidelines
3. **Service Inventory Check**: Load service-inventory.yaml from frontend/backend roots to verify reuse
4. **Adaptation Guide Review**: Check adaptation guide for mandated service reuse requirements
5. **Branch Verification**: Confirm on feature branch (NOT main) using `git branch --show-current`
...
*One block per convention this change establishes (or a single "None" line). List every convention separately — the orchestrator derives the per-convention `conventions_introduced` / `guards_shipped` / `prose_only_tagged` / `missing` arrays for the `convention_guard_check` event from these blocks, so a change that establishes two conventions where one is guarded and one is MISSING must show BOTH (a single status line cannot represent a per-convention split).*
