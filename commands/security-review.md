...
**Security-review-specific obligation:** CRITICAL and HIGH severity security findings can NEVER be deferred — there is no catastrophic condition that permits shipping a known critical/high vulnerability. Re-dispatch immediately for any such deferral. MEDIUM/LOW findings introduced by this ticket's diff must also be fixed in this ticket; pre-existing MEDIUM/LOW findings may be classified as `OUT-OF-SCOPE` with a NEW_TICKET recommendation.

If invoked standalone, this command is responsible for the same deferral validation as the orchestrators.
