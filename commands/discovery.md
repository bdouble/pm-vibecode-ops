...
|----------------|--------------|---------|
| "All data access goes through repositories" (CLAUDE.md) | grep for direct ORM imports outside repositories/ | ✅ Holds / ❌ 3 violations found |
...
- `[enforced:]` rules: X | `[prose-only]` rules: Y (discipline-debt baseline for /entropy-audit)
```

A claim that fails verification is itself a finding — the project's memory is lying to every session. Include the memory fix in the discovery output; see the `model-aware-behavior` skill (Verification Over Recall).
