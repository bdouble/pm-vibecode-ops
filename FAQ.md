...
├── src/                    # Main repo
└── .worktrees/
    ├── TICKET-123/         # Isolated copy
    └── TICKET-456/         # Another isolated copy
```
...
├── .worktrees/
│   ├── TICKET-101/            # Agent working on authentication
│   ├── TICKET-102/            # Agent working on payments
│   └── TICKET-103/            # Agent working on reports
```
...
1. You run `/adaptation TICKET-123` → Worktree created automatically
2. You run `/implementation TICKET-123` → Works in isolated directory
3. You run `/security_review TICKET-123` → Merges changes and cleans up
...
**Common causes**:
1. Skipped adaptation phase → Run `/adaptation` first
2. Manual deletion of worktree directory → Re-run `/adaptation`
3. Wrong ticket ID → Verify ticket ID matches Linear
4. Repository corruption → Remove and recreate worktree
...
Monday 9am:  PM B starts TICKET-102 (payments) ← concurrent!
