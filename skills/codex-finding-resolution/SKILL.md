...
**Report template:** See `references/report-template.md` for the full template, including the skip/error variants and the critical rules about preserving the audit trail (issue → Codex's question → Codex's recommendation → user's decision → user's reasoning).
