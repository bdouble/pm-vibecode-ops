...
<!-- @protected reason="foundational principle — the closure-log is the v4.6 anti-sprawl audit trail; silent aggregation drops are how sprawl reappears (SkillOpt §3.6 protects slow-state content from rewrites)" -->
...
|---|---|---|
| Heading-level drift | Sub-tickets use `##`, `###`, or `####` for the same section (h2 legacy, h3 canonical, h4 nested under follow-up-discipline) | Recognize ALL THREE heading levels when fetching; aggregate together |
...
2. **Extract the closure-log section** — match `(##|###|####)\s+Considered but not pursued` at line start, then capture until the next `(##|###|####)` header or end of comment. h3 is canonical (v4.6.1+ phase reports); h2 is the v4.6.0 legacy form; h4 is the form `agents/epic-closure-agent.md`'s Follow-Up Discipline template emits when the section is nested inside an h3 wrapper.
...
## Heading-Level Drift (the v4.6.0/v4.6.1/v4.7 fingerprint)

v4.6.0 templates emitted `## Considered but not pursued` (h2). v4.6.1 standardized on `### Considered but not pursued` (h3) to keep the section under the phase report's `##` header. v4.7's `epic-closure-agent` Follow-Up Discipline template emits the section at `####` (h4) as a sibling of the h4 follow-up items inside its h3 wrapper. Tickets executed across these boundaries contain a mix. Aggregation that matches only one level silently drops the others.
...
1. **Confirm by counting** — explicitly post the per-ticket scan result so the operator can audit ("Scanned 8 sub-tickets × ~5 phase comments each = 40 candidate locations across h2/h3/h4; 0 closure-log sections found.").
...
- `commands/references/observability-schema.md` — `epic_completed` envelope including `closure_log_items`
