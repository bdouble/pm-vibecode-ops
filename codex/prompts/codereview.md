...
Ask the user to provide a Linear ticket ID. Perform code quality review for the provided ticket to ensure implementation meets best practices and quality standards.
...
**STRICT REVIEW FOCUS**:
- Review ONLY code changes directly related to the Linear ticket ID and its comments
...
During code review, identify and fix any workarounds or temporary code:
...
6. **Branch Verification**: Confirm on feature branch (NOT main) using `git branch --show-current`
7. **PR Discovery**: Find existing PR for the ticket/branch using GitHub CLI
8. **Enhanced PR Comment Analysis**: Parse and categorize existing review feedback:
...
9. **Workaround Detection Scan**: Run automated detection for temporary code patterns
10. **Static Analysis**: Run automated linting and type checking
11. **Duplication Analysis**: Verify against service inventory matrix
12. **Code Quality Review**: Analyze architecture alignment, pattern consistency, business logic, addressing PR feedback
13. **Security Documentation Check**: Verify security-sensitive functions have proper JSDoc
14. **Security Issue Logging**: If security issues noticed, LOG them in Linear (do NOT fix)
15. **Fix Implementation**: Make focused commits for NON-SECURITY and DUPLICATION issues (including those from PR comments)
16. **Commit & Push**: Commit fixes to feature branch and push to remote
17. **PR State Change**: Move PR from DRAFT to READY FOR REVIEW using `gh pr ready` (BEFORE adding comments)
18. **PR Comment with Acknowledgments**: Add review results acknowledging addressed feedback
19. **PR Labels & Description**: Add `code-reviewed` label and update PR description with review summary
20. **Enhanced Linear Update**: Use structured report template with metrics dashboard
...
```javascript
// Conceptual duplication check (adapt to your language)
function detectDuplication(implementation, serviceInventory) {
  const violations = [];
  
  // Check for recreated services
  implementation.newServices.forEach(service => {
    const existing = findSimilarService(service, serviceInventory);
    if (existing) {
      violations.push({
        type: 'SERVICE_DUPLICATION',
        newService: service,
        existingService: existing,
        fix: `Replace with ${existing.name} from ${existing.location}`
      });
    }
  });
  
  // Check for custom implementations of common patterns
  implementation.utilities.forEach(util => {
    const existing = findExistingUtility(util, serviceInventory);
    if (existing) {
      violations.push({
        type: 'UTILITY_DUPLICATION',
        customImplementation: util,
        existingUtility: existing,
        fix: `Use ${existing.name} from shared utilities`
      });
    }
  });
  
  // Check for direct coupling where events exist
  implementation.dependencies.forEach(dep => {
    if (serviceInventory.eventPatterns[dep.type]) {
      violations.push({
        type: 'MISSING_EVENT_PATTERN',
        directCoupling: dep,
        availableEvent: serviceInventory.eventPatterns[dep.type],
        fix: `Use event ${dep.type} instead of direct coupling`
      });
    }
  });
...
**Review**: Automated Code Review Analysis
```
