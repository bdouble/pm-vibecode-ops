...
Ask the user for a Linear ticket ID and adapt all requirements to existing codebase patterns, creating a precise implementation guide.

When providing the Linear ticket, the user may optionally provide a discovery report ticket or document, or additional information to guide the adaptation work. 

Perform comprehensive analysis and adaptation, including discovery if no discovery report is provided.
...
**This phase is PLANNING ONLY. Do NOT:**
...
5. **Verify, don't recall**: claims in the ticket, discovery report, or CLAUDE.md/AGENTS.md about existing patterns/services/migrations are hypotheses — verify them against HEAD before building the guide on them. A claim that fails verification is a finding to report (and the stale memory gets corrected as part of this ticket).

## Vendor-Surface Discipline

Adding a vendor/SaaS dependency (npm package wrapping an external service, a new API integration, an observability vendor) requires the same justification rigor as deferring an acceptance criterion. Every external service is a behavioral coupling that will eventually change underneath the project.
...
3. **Status Update**: Use `mcp__linear-server__update_issue` to mark ticket status as "In Progress"
...
- **Convention Guards**: If this ticket establishes a convention (a pattern other code must follow, an "always/never" rule), the guide names the guard to ship with it — rung + artifact (e.g., "rung-2 source-scanning test at `tests/guards/<name>.test.ts`"); see the production-code-standards skill's Enforcement Ladder (`codex/skills/production-code-standards/SKILL.md`). The code review phase treats a mandated-but-missing guard as a SCOPE_GAP.
...
1. **Project Creation**:
```javascript
// ALWAYS create project when decomposing into subtickets
const project = await mcp__linear-server__create_project({
  name: `${parentTicket.title} Implementation`,
  description: `Project for implementing ${parentTicket.identifier}: ${parentTicket.description}`,
  teamId: parentTicket.teamId,
  leadId: parentTicket.assigneeId,
  targetDate: parentTicket.dueDate
});
```

2. **Subticket Creation with Proper Linking**:
```javascript
// Create subtickets with parent relationship
for (const area of decomposedAreas) {
  const subticket = await mcp__linear-server__create_issue({
    title: area.title, // NO parent ID in title!
    description: area.description,
    teamId: parentTicket.teamId,
    projectId: project.id, // Link to project
    parentId: parentTicket.id, // CRITICAL: Set parent relationship
...
When no discovery report is provided:
...
**Analysis**: Comprehensive analysis and adaptation performed
