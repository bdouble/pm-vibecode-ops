...
Ask the user to provide a Linear ticket ID. The user may also provide context to consider when providing the ticket ID. 
...
- Analyze ONLY security implications of code changes for the provided Linear ticket ID
...
During security review, flag any security workarounds or temporary fixes:
...
/**
 * SECURITY: Implements OAuth2 with PKCE flow.
 * Rate limited to 5 attempts per minute per IP.
 */
async function authenticate(credentials: LoginDto): Promise<Session>

// ✅ REQUIRED: Functions handling PII
/**
 * WARNING: Processes PII data - ensure GDPR compliance.
 ...
 * Uses DOMPurify with strict whitelist configuration.
 */
...
5. **Enhanced PR Security Comment Analysis**: Categorize and track security feedback:
   ...
   ```
6. **Dynamic Vulnerability Check**: Search for latest framework vulnerabilities:
   ```bash
   ...
   ```
7. **Service Inventory Security Check**: Verify security patterns in reused services:
   ```bash
   ...
   ```
8. Use a sub-task to identify vulnerabilities. Use the repository exploration tools to understand the codebase context, then analyze the PR changes for security implications. **Include any security concerns from PR comments AND Linear comments as areas to specifically investigate.** In the prompt for this sub-task, include all of the above including latest CVE patterns.
9. Then for each vulnerability identified by the above sub-task, create a new sub-task to filter out false-positives. Launch these sub-tasks as parallel sub-tasks. In the prompt for these sub-tasks, include everything in the "FALSE POSITIVE FILTERING" instructions and Anthropic's exclusion patterns.
10. Filter out any vulnerabilities where the sub-task reported a confidence less than 8.
11. **Security Fix Implementation**: If vulnerabilities found, make focused commits with clear messages following severity calibration:
...
12. **Commit & Push**: Commit security fixes to feature branch and push to remote
13. **Enhanced PR and Linear Integration**: Add structured security report with severity matrix and acknowledgments
...
Your final reply must contain the detailed vulnerability report, confirmation of PR updates, AND confirmation that Linear ticket was updated with security review comment and ticket closure status.
...
# Step 2: Extract merge target from Linear ticket comments
# The adaptation phase documents the merge target - either "main" or a feature branch
echo "Extracting merge target from Linear ticket..."

# NOTE: Linear MCP integration point - extracts merge target from ticket comments
# The adaptation phase documents merge target in Linear with pattern: **Merge Target**: `[branch-name]`
# Use mcp__linear-server__list_comments to retrieve and parse this value
#
# Current implementation: Fallback detection from git branches
# For full Linear integration, parse ticket comments for the documented merge target
...
if [ $? -ne 0 ]; then
    echo "❌ ERROR: Failed to checkout $MERGE_TARGET branch"
    echo "Manual intervention required"
    exit 1
fi

# Pull latest changes from origin
echo "Pulling latest changes from origin/$MERGE_TARGET..."
...
echo "Merging $BRANCH_NAME into $MERGE_TARGET..."
git merge --no-ff "$BRANCH_NAME" -m "Merge $TICKET_ID: Complete ticket workflow

✅ All phases complete:
- Adaptation: Implementation guide created
- Implementation: Production code written
- Testing: Comprehensive test suite added
- Documentation: API and user docs complete
- Code Review: Quality standards verified
- Security Review: No critical/high vulnerabilities

Merges to: $MERGE_TARGET
Closes $TICKET_ID"
...
if [ $? -eq 0 ]; then
    ...
    echo "Pushing merged changes to origin/$MERGE_TARGET..."
    git push origin "$MERGE_TARGET"

    if [ $? -eq 0 ]; then
        echo "✅ Changes pushed to origin/$MERGE_TARGET successfully"
        ...
        echo "Manual intervention required: git push origin $MERGE_TARGET"
        exit 1
    fi
else
    ...
    echo "4. Push changes: git push origin $MERGE_TARGET"
    echo "5. Re-run security review to complete cleanup"
...
echo ""
echo "✅ Workflow completion successful!"
echo "   - Feature branch merged to main"
echo "   - Changes pushed to origin"
echo "   - Worktree cleaned up"
echo "   - Ticket ready to be closed"
echo ""

# Step 4: Close the Linear ticket - this is the END of the workflow
# Use mcp__linear-server__update_issue to mark ticket as "Done"
echo "Marking Linear ticket as Done..."
