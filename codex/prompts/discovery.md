...
First, ask the user to provide additional context - this may be one or more of:
- PRD document (for context and requirements understanding)
- Linear project name (project to attach discovery ticket to)
- Codebase path (root directory to analyze)
- Focus areas

Systematically examine the codebase and create a comprehensive discovery ticket in Linear that all subsequent phases will reference.
...
### Initial Creation
When creating the discovery ticket:
```javascript
{
  title: "Technical Discovery - [Project Name]",
